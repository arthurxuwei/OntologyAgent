...
> v1 采用 **wallet first, owner later** 模型（沿用原型 M2 叙事）。`wallets` 出生就携带 Eigenflux Agent ID，`owner_id` 在 claim 后填入。"binding" 在 v1 即"wallet"（1:1），不再单独建表。

```
id                  uuid PK
eigenflux_agent_id  text UNIQUE          -- Flow 1 init 时落，全生命周期稳定（R.Q1）
display_name        text                  -- claim 时由 owner 填
owner_id            uuid NULL FK → owners -- Flow 2 claim 后填；NULL 表示未认领
circle_wallet_id    text                  -- Circle Gateway 钱包
address             text                  -- Base 链上地址
state               enum('unclaimed', 'claimed', 'frozen', 'disabled')
single_tx_cap_usdc  numeric(36,6)
daily_cap_usdc      numeric(36,6)
monthly_cap_usdc    numeric(36,6)
created_at          timestamptz           -- Flow 1 时间
claimed_at          timestamptz NULL      -- Flow 2 时间
disabled_at         timestamptz NULL
```

> **状态约束**：所有 lock / release / withdraw 路径强制 `state='claimed'`；`unclaimed` 钱包只能接收 deposit / onramp（资金可入但不可花）。

#### `wallet_claim_codes`（Flow 1 → Flow 2 的桥）

```
id          uuid PK
wallet_id   uuid FK → wallets
code_hash   text                          -- argon2id(claim_code 明文)
expires_at  timestamptz                    -- Flow 1 +24h
used        boolean DEFAULT false
used_at     timestamptz NULL
created_at  timestamptz
```

> 一个 wallet 同一时刻仅 1 条 unused 行（写入时旧的标记 used / expired）；明文 claim_code 不入库，仅在 Flow 1 响应里出现一次。

#### `agent_credentials`

```
id              uuid PK
wallet_id       uuid FK → wallets         -- v1: 1 wallet = 1 binding
...
```
id                uuid PK
buyer_wallet_id   uuid FK → wallets
seller_wallet_id  uuid FK → wallets
...
```
id                       bigint PK
wallet_id                uuid FK → wallets  -- 主体
event_type               text               -- 'wallet.created.unclaimed', 'wallet.claimed', 'escrow.locked', 'escrow.released', 'escrow.rejected', ...
...
#### `deposits` / `withdrawals` / `reconciliation_runs` / `platform_state`

> 略，按状态机 §5 展开；`platform_state` 是单行表，存全平台 freeze 状态（ADR-008）。

### 3.2 关键索引

- `wallets(eigenflux_agent_id)` UNIQUE
- `wallets(owner_id)` 用于 owner Console 查询自己的钱包
- `wallets(state)` 部分索引（`WHERE state IN ('claimed', 'frozen')`）用于活跃钱包扫描
- `wallet_claim_codes(wallet_id) WHERE used=false` 用于 claim 校验
- `agent_credentials(key_id)` UNIQUE
- `escrows(state, expires_at)` 用于 timer 扫描
- `events(wallet_id, occurred_at DESC)` 用于 M5 聚合
...
- **Chief Credentials (key_id + secret)**：授权用。每个 wallet 有 0–N 个 active credential。

### 4.2 Credential 颁发流程（wallet-first）

钱包归属流程见 [04-flows.md](../../docs/architecture/04-flows.md) Flow 0 / 1 / 2。简述：

```
Flow 1 — Agent 初始化创建 wallet（无 Chief credential，公开 Eigenflux Agent ID 入参）：
  1. OpenClaw plugin 启动 → 读 Eigenflux Agent ID
  2. POST /v1/wallets/init { eigenflux_agent_id }
     - eigenflux-client 验证 Eigenflux 上该 Agent ID 存在且 active
     - Circle Gateway createWallet → 落 wallets 行（owner_id=NULL, state='unclaimed'）
     - 生成 claim_code (32 bytes random base32)，DB 仅存 argon2id hash
  3. 响应里返回 claim_code 明文一次 → plugin 终端显示给用户

Flow 0 — 用户 OAuth 登录（任意时刻发生）：
  4. GitHub OAuth → owners 表 → session cookie

Flow 2 — 用户 claim wallet（Flow 0 + Flow 1 都完成后）：
  5. 用户在 Console /claim 页粘贴 claim_code
  6. POST /v1/wallets/claim { claim_code }
     - argon2id verify code_hash
     - BEGIN; UPDATE wallets SET owner_id=session.owner_id, state='claimed', claimed_at=now()
       UPDATE wallet_claim_codes SET used=true; INSERT events; COMMIT
     - 邮件通知 owner，24h 反 claim 窗口（mainnet 头月）

Flow 3 — Plugin 取 credential（OAuth device-code，wallet 必须 state='claimed'）：
  7. POST /v1/oauth/device/authorize { eigenflux_agent_id }
  8. 用户在浏览器输入 user_code 确认 scope
  9. 生成 (key_id, secret)，secret 仅响应里一次；DB argon2id hash
  10. Plugin 写入 OS keychain，后续走 §4.3 HMAC 签名
```

> 同一个 wallet 可以挂多个 active credential（多机器 / 多环境），按 §4.4 lifecycle 各自独立旋转。
> Flow 1 / Flow 2 的安全风险（claim_code 抢跑）在 §11 T10 中处理。
...
| Owner kill-switch | Owner 触发 | `owner.disabled_at` 设置；该 owner 下所有 wallet 的 credential 全部失效 |
...
| `GET /v1/wallets` | 列出当前 owner 已 claim 的钱包 |
| `POST /v1/wallets/init` | **公开端点**（plugin 调，无 Chief credential）：按 Eigenflux Agent ID 创建未认领钱包 + 返回 claim_code 一次 |
| `POST /v1/wallets/claim` | 用 claim_code 把 unclaimed wallet 绑定到当前 session 的 owner |
| `POST /v1/wallets/{id}/reverse-claim` | mainnet 头月：claim 后 24h 内 owner 可强制反转 claim（GitHub OAuth + TOTP 双因子） |
| `POST /v1/wallets/{id}/credentials` | 颁发 / 旋转 credential（管理用，OpenClaw 走 device-code flow） |
| `DELETE /v1/wallets/{id}/credentials/{key_id}` | 撤销 credential |
...
```
定义：
  N_total   = 该 wallet 作为 seller 在过去 30 天内进入终态（RELEASED 或 REFUNDED）的 escrow 总笔数
...
freeze 含义：
  Chief 拒绝该 wallet 作为 seller 的所有新 escrow lock 请求
...
| Per-wallet | Owner | 该 wallet 所有 credential 拒收（v1 即"per agent"，因 1:1） |
...
| 调用点 | 用途 | 缓存策略 |
|---|---|---|
| Wallet init（Flow 1） | 验证 Eigenflux Agent ID 存在 | 不缓存（一次性） |
...
| T9 | Device-code flow 被钓鱼（attacker 偷 user_code 引诱 owner 确认） | user_code 短 TTL（≤ 10 分钟）+ Console 显示 "你正在为哪个 wallet 授权 + scope 列表" 让 owner 主动核对 + 同一 owner 同一时段并发 device flow 数限制 |
| T10 | claim_code 抢跑：attacker 知道 victim 的 Eigenflux Agent ID，先于 victim 调 `/v1/wallets/init` 拿到 claim_code，再用自己 OAuth session 走 `/v1/wallets/claim` 占走钱包 | (1) `wallets.eigenflux_agent_id` UNIQUE → victim 后续 init 立刻报错 "wallet_already_exists"，异常**立即可见**； (2) claim_code 24h TTL + 一次性 → 攻击窗口有限； (3) **mainnet 头月**：claim 成功强制邮件通知 + 24h 反 claim 窗口（GitHub OAuth + TOTP 双因子撤销并重发新 claim_code）； (4) 长期方案 v1.1：等 Eigenflux 加密码学认证后，init 要求 Eigenflux 签名 envelope |
...
- **T10 claim_code 抢跑的反 claim 窗口**：mainnet 头月开 24h 反 claim 是必做；之后是关掉、缩短到 1h，还是改成"事后人工 dispute" —— 等头月真实数据再定 |
